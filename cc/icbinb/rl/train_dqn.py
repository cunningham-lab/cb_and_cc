...
class Agent(object):
    ...
    def learn(self, batch_size, gamma, frame_number, priority_scale=1.0):
        ...
        loss, error = self.compute_apply_gradients(batch_size, gamma, states, actions, rewards, new_states, terminal_flags, importance, indices)

        return loss, error

    @tf.function
    def compute_apply_gradients(self, batch_size, gamma, states, actions, rewards, new_states, terminal_flags, importance, indices):
        # Main DQN estimates best action in new states
        arg_q_max = tf.math.argmax(self.DQN.call(new_states), axis=1)
        # arg_q_max = self.DQN.predict(new_states).argmax(axis=1)

        # Target DQN estimates q-vals for new states
        # future_q_vals = self.target_dqn.predict(new_states)
        future_q_vals = self.target_dqn.call(new_states)
        # double_q = future_q_vals[np.arange(batch_size), arg_q_max]
        idxs = tf.stack((tf.range(0, batch_size, dtype='int64'), arg_q_max), -1)
        double_q = tf.gather_nd(future_q_vals, idxs)
    ...
    def load(self, folder_name, load_replay_buffer=False):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ENV_NAME', dest='ENV_NAME', type=str, default=ENV_NAME)

    args = parser.parse_args()
    ENV_NAME = args.ENV_NAME
    SAVE_PATH = 'saves/' + ENV_NAME
    ...
    # TensorBoard writer
    writer = tf.summary.create_file_writer(TENSORBOARD_DIR + ENV_NAME + '/' + datetime.now().strftime("%Y-%m-%d-%H%M%S"))
                ...
                while epoch_frame < FRAMES_BETWEEN_EVAL:
                    ...
                    for _ in range(MAX_EPISODE_LENGTH):
                        ...
                        if frame_number % UPDATE_FREQ == 0 and agent.replay_buffer.count > MIN_REPLAY_BUFFER_SIZE:
                            ...
                            f_n = tf.cast(frame_number, 'int32')
                            loss, _ = agent.learn(BATCH_SIZE, gamma=DISCOUNT_FACTOR, frame_number=f_n, priority_scale=PRIORITY_SCALE)
